@if(is_plugin_active('results/results.php'))
  <div class="section results">
    <ul class="results__type nav nav-tabs" id="resultsTabNav" role="tablist">
      @foreach ($results as $key => $type)
        @if($type['type'])
          <li class="results__type-item" role="presentation">
            <a class="results__type-link {{ ($loop->first) ? 'active' : '' }}" data-bs-toggle="tab" data-bs-target="#resultsType-{{ $type['type']['slug'] }}-pane" type="button" role="tab" aria-controls="resultstype-{{ $type['type']['slug'] }}-pane" aria-selected="{{ ($loop->first) ? 'true' : 'false' }}">{{ $type['type']['name'] }}</a>
          </li>
        @endif
      @endforeach
    </ul>
    ...
    @if($results)
      <div class="results__type-content tab-content" id="resultsTypeTabContent">
        @foreach($results as $key => $type)
          <div class="results__type-pane tab-pane fade {{ ($loop->first) ? 'show active' : '' }}" id="resultsType-{{ $type['type']['slug'] }}-pane" role="tabpanel" aria-labelledby="resultsType-{{ $type['type']['slug'] }}-tab" tabindex="0">
            @if($type['yearlist'])
              <ul class="results__nav table-tabs nav nav-tabs" id="resultsTabNav" role="tablist">
                @foreach ($type['yearlist'] as $yKey => $item)
                  @if($item['years'])
                    <li class="results__nav-item table-tabs__nav-item" role="presentation">
                      <a class="results__nav-link table-tabs__nav-link {{ ($loop->first) ? 'active' : '' }}" data-bs-toggle="tab" data-bs-target="#results-{{ $key . '-' . $yKey . '-' . $item['years']['slug'] }}-pane" type="button" role="tab" aria-controls="results-{{ $key . '-' . $yKey . '-' . $item['years']['slug'] }}-pane" aria-selected="{{ ($loop->first) ? 'true' : 'false' }}">{{ $item['years']['name'] }}</a>
                    </li>
                  @endif
                @endforeach
              </ul>
            ...
            <div class="results__tab-content table-tabs__tab-content tab-content" id="resultsTabContent">
              @foreach($type['yearlist'] as $yKey => $item)
                <div class="results__tab-pane table-tabs__tab-pane tab-pane fade {{ ($loop->first) ? 'show active' : '' }}" id="results-{{ $key . '-' . $yKey . '-' . $item['years']['slug'] }}-pane" role="tabpanel" aria-labelledby="results-{{ $key . '-' . $yKey . '-' . $item['years']['slug'] }}-tab" tabindex="0">
                  {{-- Links --}}
                  @if($item['results'])
